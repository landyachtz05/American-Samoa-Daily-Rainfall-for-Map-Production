...
out_dir  <- file.path(main_dir, "as_idw_monthly_rf_table_NRT")
...
monthly_df <- daily %>%
  select(SKN, Station.Name, monthly_sum)
...
if (file.exists(annual_file)) {
  ...
  annual_df <- annual_df |>
    select(-any_of(setdiff(metadata_cols, c("SKN","Station.Name")))) |>
    left_join(
      daily |> select(all_of(metadata_cols)) |> distinct(),
  ...
  annual_df <- daily |> select(all_of(metadata_cols)) |> distinct()
...
final_df <- annual_updated |>
  select(all_of(metadata_cols), all_of(month_cols))
